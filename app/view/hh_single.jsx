...
import HhSingle from '../assets/rhyme/hh_single/HhSingle.jsx';
...
export default function(data) {
  ...
  let hhSingle = migi.preRender(
    <HhSingle single={ single }/>
  );
...
<head>
  ...
  <link rel="stylesheet" href="${data.helper.getAssetUrl('/hh_single.css')}"/>
</head>
<body>
<div id="page">${hhSingle}</div>
<script>
  ${data.helper.$CONFIG}
  $CONFIG.single = ${data.helper.stringify(single)};
</script>
<script src="${data.helper.getAssetUrl('/rcommon.js')}" defer="defer"></script>
<script src="${data.helper.getAssetUrl('/hh_single.js')}" defer="defer"></script>
${data.helper.getStat()}
</body>
</html>`;
}
